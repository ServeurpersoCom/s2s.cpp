...
static float * read_wav_buf(const uint8_t * data, size_t size, int * T_audio, int * sr) {
    ...
    *sr      = 0;
    ...
    if (size < 12 || memcmp(data, "RIFF", 4) != 0 || memcmp(data + 8, "WAVE", 4) != 0) {
        s2s_log(S2S_LOG_ERROR, "[WAV] Not a valid WAV buffer");
        return NULL;
    }
    ...
    while (pos + 8 <= size) {
        ...
        if (memcmp(chunk_id, "fmt ", 4) == 0 && chunk_size >= 16) {
            ...
            if (audio_format == 1 && bits_per_sample == 16) {
                ...
                if (!audio) {
                    s2s_log(S2S_LOG_ERROR, "[WAV] OOM allocating PCM16 buffer for %d samples", n_samples);
                    return NULL;
                }
                ...
                audio     = (float *) malloc((size_t) n_samples * 2 * sizeof(float));
                if (!audio) {
                    s2s_log(S2S_LOG_ERROR, "[WAV] OOM allocating PCM24 buffer for %d samples", n_samples);
                    return NULL;
                }
                const uint8_t * p = data + pos;
                ...
                audio     = (float *) malloc((size_t) n_samples * 2 * sizeof(float));
                if (!audio) {
                    s2s_log(S2S_LOG_ERROR, "[WAV] OOM allocating F32 buffer for %d samples", n_samples);
                    return NULL;
                }
                const uint8_t * p = data + pos;
                ...
                s2s_log(S2S_LOG_ERROR, "[WAV] Unsupported: format=%u bits=%d subformat=%u", (unsigned) audio_format,
    ...
    if (!audio) {
        s2s_log(S2S_LOG_ERROR, "[WAV] No audio data in buffer");
        return NULL;
    }

    *T_audio = n_samples;
    *sr      = sample_rate;
    s2s_log(S2S_LOG_INFO, "[WAV] Read buffer: %d samples, %d Hz, %d ch, %d bit", n_samples, sample_rate, n_channels,
            bits_per_sample);
    return audio;
}
